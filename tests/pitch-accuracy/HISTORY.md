...
## Phase 122: Feedback Pattern Correction (2026-01-05)

### 배경
- Phase 121 실패 후 새로운 접근 필요
- **피드백 수집 시스템** 구축 완료 (`lib/feedbackCollection.ts`)
- 기존 배치 테스트 데이터를 FeedbackSession 형식으로 변환하여 패턴 분석

### 피드백 데이터 분석 결과

**변환 스크립트**: `tests/pitch-accuracy/convertToFeedback.ts`
...
| C3 → D3 | 8회 | 2반음 상향 오류 |
| F#3 → G3 | 5회 | 1반음 상향 오류 |
| F3 → F#2 | 4회 | **옥타브 오류** (11반음) |

### 구현 내용

#### Phase 118 개선: FRAGMENT_THRESHOLD 상향
```typescript
// pitchToNote.ts:1458
const FRAGMENT_THRESHOLD_118 = 3; // 2 → 3 (3슬롯도 병합 대상)
```
- 피드백 패턴: 3슬롯→4슬롯 오류 18회
- 3슬롯 음표도 fragment로 취급하여 병합 기회 증가

#### Phase 122-A: 유사 음정 병합 (±1반음)
```typescript
// pitchToNote.ts:1971-2006
// 피드백 패턴: C3→D3(8회), F#3→G3(5회) 등 반음 오류가 빈번
// 연속된 짧은 음표(≤2슬롯)가 ±1반음 이내이면 병합
if (pitchDiff <= 1 && gap <= 1 && currNote.slotCount <= 2 && prevNote.slotCount <= 2) {
  // 병합: 이전 음표 피치 유지
}
```
...
```typescript
// pitchToNote.ts:2009-2039
// 피드백 패턴: F3→F#2(11반음), G3→A2(10반음) 등
// 평균 음정에서 ±10~14반음 벗어난 음표 감지 → 옥타브 보정
if (diff >= 10 && diff <= 14) {
  correctedMidi = noteMidi < avgMidi ? noteMidi + 12 : noteMidi - 12;
}
```

#### Phase 116 테스트: 3슬롯→4슬롯 강제 양자화
```typescript
// 피드백 패턴: 3슬롯→4슬롯 18회 오류
if (note.slotCount === 3) {
  note.slotCount = 4; // 4분음표로 양자화
}
```
...
**원인**: 실제로 3슬롯이 정답인 케이스도 있음
- case_06: 61.5% → 53.8% (-7.7%)
- case_07: 77.8% → 70.4% (-7.4%)

→ **Phase 116 비활성화** (롤백)

### 배치 테스트 결과

| Case | Notes | Match | Pitch | Timing | Duration | Overall |
|------|-------|-------|-------|--------|----------|---------|
| case_02 | 28 | 26 | 84.6% | 84.6% | 57.7% | 75.6% |
| case_03 | 28 | 25 | 84.0% | 96.0% | 72.0% | 84.0% |
| case_04 | 27 | 23 | 73.9% | 73.9% | 56.5% | 68.1% |
| case_05 | 16 | 16 | 56.3% | 100.0% | 31.3% | 62.5% |
| case_06 | 16 | 13 | 100.0% | 61.5% | 61.5% | 74.4% |
| case_07 | 32 | 27 | 88.9% | 70.4% | 77.8% | 79.0% |
| case_08 | 27 | 21 | 61.9% | 71.4% | 28.6% | 54.0% |
| case_09 | 27 | 23 | 82.6% | 87.0% | 65.2% | 78.3% |

**전체 평균 (가중)**:
| 지표 | 값 | 목표 | 상태 |
|------|-----|------|------|
| 음정 정확도 | **79.3%** | 80% | 거의 달성 |
| 타이밍 정확도 | **81.0%** | - | ✅ 달성 |
| 길이 정확도 | **58.0%** | - | 병목 |
| **종합 정확도** | **72.8%** | **80%** | 7.2% 필요 |

### 교훈

1. **피드백 패턴 기반 접근은 유효** - Phase 122-A/B 구현 완료
2. **강제 양자화는 위험** - 3슬롯→4슬롯 변환이 역효과
3. **Duration 개선의 어려움** - 파라미터/규칙 조정으로는 한계
4. **실제 사용자 피드백 필요** - 더 많은 데이터로 패턴 정교화
...
| 파일 | 내용 |
|------|------|
| `tests/pitch-accuracy/convertToFeedback.ts` | 테스트→FeedbackSession 변환 |
| `tests/pitch-accuracy/feedbackSessions.json` | 변환 결과 저장 |
| `utils/pitchToNote.ts:midiToPitch()` | MIDI→Pitch 변환 함수 |
| `utils/pitchToNote.ts:Phase 122` | 피드백 패턴 보정 로직 |

### 향후 방향

1. **실제 사용자 피드백 수집** - Supabase feedback_sessions 테이블 활용
2. **패턴 기반 자동 튜닝** - 충분한 데이터 축적 후 ML 적용 검토
3. **Duration 근본 해결** - Envelope 분석 또는 spectral approach 연구
4. **Interactive Smart Guide** - 저신뢰 구간 사용자 직접 입력 (Phase 122 이전에 구현 완료)


---

## 수동 파라미터 튜닝 세션 (2026-01-06)

### 목적
Duration 및 Pitch 정확도 개선을 위한 파라미터 튜닝 테스트

### 테스트 케이스: 10개
- case_02~case_11 (총 308개 음표)
...
| 시도 | 파라미터 | 변경 | 결과 | 비고 |
|------|---------|------|------|------|
| 1 | MAX_MERGE_SLOTS | 6→8 | 71.0% | -1.0% 악화, 과잉 병합 |
| 2 | MAX_MERGE_SLOTS | 6→12 | 69.9% | -2.1% 악화, 심각한 과잉 병합 |
| 3 | OCCUPANCY_SUSTAIN | 0.25→0.35 | 72.0% | 변화 없음 |
| 4 | ONSET_ENERGY_RATIO | 1.05→1.20 | 72.0% | 변화 없음 |
| 5 | ONSET_DETECTION_ENABLED | true→false | **66.7%** | **-5.3% 붕괴!** |

### 케이스별 상세 분석

**case_05 (Duration 31.3%, Pitch 56.3%)**
- 16슬롯 긴 음표가 3-4슬롯으로 분할됨
- 평균 옥타브 2 → 목표 옥타브 3 시프트 적용
- 앵커 포인트: D#2, E2, F2, D2, C#2, F#2 (313개)
- 문제: 피치 감지 자체의 오류 (파라미터 문제 아님)

**case_08 (Duration 28.6%, Pitch 61.9%)**
- 4슬롯 음표가 1슬롯으로 감지됨
- 6개 음표 미검출, 6개 과잉 검출
- 문제: 저음역대(D2-A2) 피치 감지 불안정

### 핵심 발견

1. **Onset Detection은 필수**
   - 비활성화시 5.3% 하락 (72.0% → 66.7%)
   - 에너지 급증 감지가 타이밍 정확도에 중요

2. **MAX_MERGE_SLOTS=6 최적**
   - 증가시 과잉 병합으로 정확도 하락
   - case_07 등에서 음표 합쳐짐 발생

3. **Duration 개선 한계**
   - 파라미터 조정으로는 59.6%에서 개선 어려움
   - 근본적인 envelope 분석 필요

4. **Pitch 오류 패턴**
   - case_05, case_08, case_10, case_11 모두 55-62% 수준
   - 저음역대(C2-G2) 감지 불안정이 주원인

### 결론

현재 황금 설정(3차)이 최적. 추가 개선을 위해서는:
- MPM/YIN 피치 감지 알고리즘 개선
- 저음역대 전용 보정 로직 강화
- 실제 사용자 피드백 데이터 축적
...
{
  "MAX_MERGE_SLOTS": 6,
  "OCCUPANCY_SUSTAIN": 0.25,
  "ONSET_ENERGY_RATIO": 1.05,
  "ONSET_DETECTION_ENABLED": true
}
```
