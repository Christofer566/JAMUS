 ...
 * @param {string} [weekFilter] - 주차 필터 (예: "W05", optional)
 * @returns {Promise<Object>} 커밋 분석 결과
 */
export async function analyzeGitHubCommits(taskNumber, weekFilter = null) {
  console.log(`\n=== Task ${taskNumber} GitHub 커밋 분석 시작 ===`);
  if (weekFilter) {
    console.log(`  - Week 필터: ${weekFilter}`);
  }
  ...
  try {
    ...
    // Task 번호가 포함된 커밋만 필터링 (Week 필터 옵션)
    console.log(`[Step 3/6] Task ${taskNumber} 커밋 필터링 중...`);
    // 패턴: [WXX] Task X.Y: (Week 부분은 optional)
    const taskPattern = new RegExp(`(?:\\[W\\d+\\]\\s*)?Task ${taskNumber}(\\.\\d+)*:`, 'i');
    let taskCommits = commits.filter(commit =>
      taskPattern.test(commit.commit.message)
    );

    // Week 필터가 있으면 추가 필터링
    if (weekFilter) {
      const weekPattern = new RegExp(`\\[${weekFilter}\\]`, 'i');
      taskCommits = taskCommits.filter(commit =>
        weekPattern.test(commit.commit.message)
      );
      console.log(`  - Week ${weekFilter} 필터 적용됨`);
    }
