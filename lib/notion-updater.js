...
const TASK_EXECUTION_LOG_DB = process.env.NOTION_UPDATE_LOGS_DB_ID;
const WEEKLY_TASK_LIST_DB = process.env.NOTION_WEEKLY_TASK_DB_ID;
const DEBUGGING_HISTORY_DB = process.env.NOTION_DEBUG_HISTORY_DB_ID;

/**
 * Task Execution Log에 새 항목 추가
 */
export async function updateNotionTaskLog(taskNumber, analysisData) {
  const { commitAnalysis, timeAnalysis, bugAnalysis } = analysisData;

  try {
    // 기존 Task 항목이 있는지 확인
    const existingPages = await notion.databases.query({
      database_id: TASK_EXECUTION_LOG_DB,
      filter: {
        property: 'Task',
        number: {
          equals: taskNumber
    ...
    if (existingPages.results.length > 0) {
      ...
      await notion.pages.update({
        ...
        properties: {
          ...
          '총 커밋': {
            number: commitAnalysis.totalCommits
          },
          '버그 수정': {
            number: commitAnalysis.bugFixCommits.length
          },
          '총 개발 시간': {
            rich_text: [{
              text: { content: timeAnalysis.totalDevelopmentTime }
            }]
          },
          'AI 구현 시간': {
            rich_text: [{
              text: { content: timeAnalysis.aiImplementationTime }
            }]
          },
          '리뷰/수정 시간': {
            rich_text: [{
              text: { content: timeAnalysis.humanReviewTime }
            }]
          }
        }
      });
      ...
      const response = await notion.pages.create({
        parent: { database_id: TASK_EXECUTION_LOG_DB },
        properties: {
          'Task': {
            number: taskNumber
          },
...
/**
 * Weekly Task List에서 Task 체크박스 업데이트
 */
