...
export async function updateNotionTaskLog(taskNumber, analysisData, weekString) {
  ...
  try {
    ...
    // 안전하게 문자열로 변환 (객체가 전달되면 JSON 문자열로 변환)
    ...
    const h2 = (content) => ({ type: 'heading_2', heading_2: { rich_text: [{ text: { content: ensureString(content) } }] } });
    const h3 = (content) => ({ type: 'heading_3', heading_3: { rich_text: [{ text: { content: ensureString(content) } }] } });
    const li = (content) => ({ type: 'bulleted_list_item', bulleted_list_item: { rich_text: [{ text: { content: ensureString(content) } }] } });
    const p = (content) => ({ type: 'paragraph', paragraph: { rich_text: [{ text: { content: ensureString(content) } }] } });
    const div = () => ({ type: 'divider', divider: {} });
    const code = (text, language = 'javascript') => ({ type: 'code', code: { rich_text: [{ text: { content: ensureString(text) } }], language } });
  ...
  try {
    ...
    for (const block of blocks.results) {
      if (block.type === 'to_do') {
        const text = block.to_do.rich_text[0]?.plain_text || '';
        if (text.includes(taskIdentifier)) {
          // Task 제목 파싱 (예: "[W05] Task 1: 제목" 또는 "Task 1: 제목")
          const titleMatch = text.match(/(?:\[W\d+\]\s*)?Task \d+(?:\.\d+)?:\s*(.+)/);
