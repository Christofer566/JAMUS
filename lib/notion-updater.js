...
export async function updateNotionTaskLog(taskNumber, analysisData, weekString) {
  ...
  try {
    // 1. 페이지 찾기 또는 생성 (임시: 항상 새 페이지 생성)
    console.log('진단용: 기존 페이지를 찾지 않고 항상 새 페이지를 생성합니다.');
    const newPage = await notion.pages.create({
      parent: { database_id: TASKS_DB },
      properties: {
        '이름': { title: [{ text: { content: `Task ${taskNumber} (Test)` } }] },
        'Task': { number: taskNumber },
      }
    });
    const pageId = newPage.id;
    console.log('새 Task Log 생성 (진단용):', pageId);
